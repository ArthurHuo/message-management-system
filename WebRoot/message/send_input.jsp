    ...
    <form id="form" name="form" method="post" action="MessageServlet?m=add">
    	<table>
    		<tr>
    			<td width = "100">
    				<label>收件人
			           <span class="small">多个收件人以逗号进行分隔</span>
			       </label>
    			</td>
    			<td>
    			
    				<%if(request.getMethod().equals("reply")){ %>
    				<input type="text" id="receiver" name="receiver" maxlength="255" size="70" value="<%=(message!=null)?message.getSender().getId()+";":"" %>"/>
    			<%}else{%>
    		...
    		<tr>
    			...
    			<td>
    				...
    				<%if(request.getMethod().equals("reply")){ %>
    				<input type="text" name="title" maxlength="255" size="70" value="<%=(message!=null)?"回复:"+message.getTitle():"" %>"/>
    			<%}else{%>
    		...
    		<tr>
    			...
    			<td>
    				<!--  <textarea rows="10" cols="60" name="content" id="editor"><%=(message!=null)?message.getContent():"" %></textarea> -->
